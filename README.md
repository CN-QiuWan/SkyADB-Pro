...
- **虚拟遥控器**：模拟返回、主页、方向键、音量、媒体控制等常用按键，支持自定义组合键。
- **应用管理**：应用列表、分类筛选、搜索、启动、停止、卸载，支持多选批量卸载 / 导出。
- **本机应用**：列出本机用户应用，导出并安装到目标设备。
- **安装 APK**：选择本机 APK 安装到已连接设备。
- **在线下载**：从 HTTP / HTTPS 链接下载 APK 或文件，支持进度、取消、断点续传、下载后安装或推送。
- **文件管理**：浏览目标设备目录、上传、下载、新建文件夹、删除文件，支持多选批量传输。
- **查看摄像头**：查看目标设备前置 / 后置摄像头画面。
- **媒体库**：浏览目标设备媒体文件。
- **系统状态**：网络、电池、存储、CPU、内存、屏幕亮度、当前应用与进程列表实时监控。
- **系统日志**：读取目标设备系统日志并支持复制。
- **Shell**：执行 Shell 命令，展示输出、错误输出与退出码，支持常用命令收藏。
- **连接分享**：为当前连接地址生成二维码，扫码即可 `adb connect`。
- **诊断日志**：记录错误和异常信息，便于排查连接和功能异常。
- **版本更新**：设置页检查 GitHub Releases 最新版本，支持应用内直接下载并安装更新。
- **设置**：默认端口、连接超时、命令超时、扫描网段、镜像画质（流畅 / 均衡 / 高清 / 自定义）、摄像头画质、状态刷新间隔、主题模式、语言（中文 / English）、清理最近设备。
- **内存优化**：限制镜像视频解码缓冲，避免长时间镜像导致的内存占用过高。
...
- **SkyADB-Pro-0.2.1-1-release.apk**可前往 Releases 页面获取最新版本。
-安全校验码
 SkyADB-Pro-0.2.1-1-release.apk
SHA-256: 162d5a92fa97762365c97bf9d81703041962a83831d550531bcf7f73b5c789f8
