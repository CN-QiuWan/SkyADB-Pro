...
class ScrcpySession private constructor(
    ...
    /** 摄像头手电筒（后置闪光灯）开关；走会话自身的 IO 作用域发送控制消息。 */
    fun setTorch(enabled: Boolean) {
        scope.launch { runCatching { controlClient.setTorch(enabled) } }
    }
    ...
    companion object {
        ...
        suspend fun start(
            ...
            cameraOptions: CameraCaptureOptions? = null,
            ...
            // 每个会话使用独立 JAR 路径，双摄并发时互不覆盖。
            val serverPath = serverManager.pushServer(controlKadb, scid)
            val socketName = "scrcpy_${ScrcpyConstants.formatScid(scid)}"
            val serverCommand = if (cameraOptions != null) {
                serverManager.buildCameraStartCommand(scid, cameraOptions, serverPath)
            } else {
                serverManager.buildStartCommand(scid, options, audioEnabled, serverPath)
            }
            val serverStream = controlKadb.open(
                "shell:$serverCommand 2>&1",
            )
